...
public class Interpol {
     ...
     * <b>post:</b> tiempoJuego = 0, ciudadRaiz = null, ciudadActual = null, 
     * caso = "", sospechosos = new ArrayList, generadaOrdenCaptura = false<br>
     */
    @SuppressWarnings("rawtypes")
    public Interpol() {
        tiempoJuego = 0;
     ...
     * @throws CiudadNoAgregadaException Excepción lanzada cuando la ciudad donde se quiere agregar la nueva ciudad ya tiene a sus hijos llenos
     */
    @SuppressWarnings("rawtypes")
    public void cargarJuego(Properties propiedades) throws InterpolException, CiudadNoAgregadaException {
        ciudadRaiz = null;
        ciudadActual = null;
        sospechosos = new ArrayList();
     ...
     * <b>post:</b> Se agregan a la lista de sospechosos todos los que se 
     * encuentren en el archivo de propiedades.<br>
     * @param propiedades Diferente de null.
     */
    @SuppressWarnings("unchecked")
    private void cargarSospechosos(Properties propiedades) throws InterpolException {
        int cantidadSospechosos = Integer.parseInt(propiedades.getProperty(
                PROPIEDADES_NUMERO_SOSPECHOSO));
        for(int i = 0; i < cantidadSospechosos; i++) {
            String rutaImagen = propiedades.getProperty(PROPIEDADES_SOSPECHOSO + 
                    (i + 1) + PROPIEDADES_SOSPECHOSO_IMAGEN);
            boolean esLadron = Boolean.parseBoolean(propiedades.getProperty(
                    PROPIEDADES_SOSPECHOSO + (i + 1) + PROPIEDADES_SOPECHOSO_ES_LADRON));
     ...
     * <b>post:</b> Se creó el árbol de ciudades, cada una de ellas con tres 
     * lugares para buscar pistas.<br>
     * @param propiedades Diferente de null.
     * @throws InterpolException Lanzada cuando se intenta agregar una ciudad 
     *         que ya existe.
     * @throws CiudadNoAgregadaException Excepción lanzada cuando la ciudad donde 
     *         se quiere agregar la nueva ciudad ya tiene a sus hijos llenos
     */
    private void cargarCiudades(Properties propiedades) throws InterpolException, 
            CiudadNoAgregadaException {
        int cantidadCiudades = Integer.parseInt(propiedades.getProperty(
                PROPIEDADES_CANTIDAD_CIUDADES));
        for(int i = 0; i < cantidadCiudades; i++) {
            String nombreCiudad = propiedades.getProperty(PROPIEDADES_CIUDAD + 
                    (i + 1) + PROPIEDADES_NOMBRE_CIUDAD);
            String descripcion = propiedades.getProperty( PROPIEDADES_CIUDAD + 
                    ( i + 1 ) + PROPIEDADES_DESCRIPCION_CIUDAD );
            if(nombreCiudad == null || descripcion == null) {
                throw new InterpolException("La información del archivo no está "
                        + "bien definida");
            }
            if(darCiudad(nombreCiudad) == null) {
                int tiempoViaje = Integer.parseInt(propiedades.getProperty(
                        PROPIEDADES_CIUDAD + (i + 1) + PROPIEDADES_TIEMPO_VIAJE));
                int cantLugares = Integer.parseInt( propiedades.getProperty(
                        PROPIEDADES_CIUDAD + (i + 1) + PROPIEDADES_LUGAR +
                        PROPIEDADES_CANTIDAD_LUGARES));
                boolean estaSospechoso = Boolean.parseBoolean(propiedades.getProperty(
                        PROPIEDADES_CIUDAD + (i + 1) + 
                        PROPIEDADES_ESTA_SOSPECHOSO));
                Ciudad ciudad = new Ciudad(nombreCiudad, descripcion, tiempoViaje, 
                        estaSospechoso);
                for(int j = 0; j < cantLugares; j++) {
                    Lugar lugar = new Lugar(propiedades.getProperty(PROPIEDADES_CIUDAD + 
                            (i + 1) + PROPIEDADES_LUGAR + (j + 1) + 
                            PROPIEDADES_NOMBRE_LUGAR ), propiedades.getProperty(
                            PROPIEDADES_CIUDAD + (i + 1) + PROPIEDADES_LUGAR + 
                            (j + 1) + PROPIEDADES_PISTA_LUGAR), Integer.parseInt(
                            propiedades.getProperty(PROPIEDADES_CIUDAD + 
                            (i + 1) + PROPIEDADES_LUGAR + (j + 1) + 
                            PROPIEDADES_TIEMPO_LUGAR)));
                    ciudad.agregarLugar( lugar );
                }
                agregarCiudad(ciudad, propiedades.getProperty(PROPIEDADES_CIUDAD + 
                        (i + 1) + PROPIEDADES_PADRE));
            }
            else {
                throw new InterpolException("Una de las ciudades que desea agregar "
                        + "ya existe");
            }
        }
    }
    ...
    /**
     * Agrega una nueva ciudad al mundo. Si la ciudad raíz no existe, se agrega 
     * la ciudad como raíz y se deja como ciudad actual.<br>
     * <b>pre:</b> La ciudad a agregar no existe<br>
     * <b>post:</b> Se agrega una nueva hoja al árbol, debajo de la que se indica 
     * en el nombrePadre<br>
     * @param ciudad Ciudad que se quiere agregar. Diferente de null.
     * @param nombrePadre Nombre de la ciudad padre a la cual se le quiere agregar 
     *        la ciudad. 
     * @throws CiudadNoAgregadaException Excepción lanzada cuando la ciudad donde 
     *         se quiere agregar la nueva ciudad ya tiene a sus hijos llenos
     */
    private void agregarCiudad(Ciudad ciudad, String nombrePadre) throws 
            CiudadNoAgregadaException {
        if(ciudadRaiz == null) {
            ciudadRaiz = ciudad;
            ...
            if(!ciudadRaiz.existeCiudad(nombrePadre))
                throw new CiudadNoAgregadaException("No se encontró la ciudad " + 
                        nombrePadre + " donde se quiere agregar la nueva ciudad");
     ...
     * @return Retorna la ciudad cuyo nombre es el que se pasa por parámetro, 
    ...
    public List darNombresPosiblesDestinos() {
        return ciudadActual != null ? ciudadActual.darNombresPosiblesDestinos() : 
                new ArrayList();
    }

    /**
     * Cambia la ciudad actual por la ciudad cuyo nombre se pasa por parámetro. <br>
     * <b>post:</b>La ciudad actual cambia. El tiempo de juego se reduce en la 
     * cantidad de días de viaje que tiene la nueva ciudad<br>
     ...
     * @return Retorna true en caso que el sospechoso se encuentre en la ciudad 
    ...
    /**
     * Retorna el camino de la raíz (primera ciudad) a la ciudad que tiene el 
     * sospechoso.<br>
     * @return Lista con los nombres de las ciudades que se encuentran entre la 
     *         raíz y la ciudad que tiene el sospechoso. Diferente de null.
     */
    @SuppressWarnings("rawtypes")
     ...
     * @return Retorna true en caso que la ciudad actual es una hoja, false de lo 
     *         contrario.
     */
     ...
     * @return True en caso que la orden de captura fuese generada, false de lo 
     * contrario.
     */
    ...
    /**
     * Retorna si el sospechoso en la posición dada es el ladrón o no, en caso 
     * que lo sea genera la orden de captura <br>
     * <b>pre:</b> Lista de sospechosos inicializada <br>
     * <b>pos:</b> En caso que el sospechoso dado sea el ladrón, generarOrdenCaptura = true <br>
     * @param pos Posición en la cual se quiere buscar el ladrón. Pos es mayor 
     *        igual a cero y menor que el tamaño de la lista de sospechosos.
     * @return True en caso que el sospechoso de la posición en la lista de 
     *         sospechosos sea el ladrón, false de lo contrario.
     */
    public boolean generarOrdenCaptura(int pos) {
        return ((Sospechoso)sospechosos.get(pos)).esLadron() ? generadaOrdenCaptura = 
                true : false;
    }
    ...
    // ------------------------------------------------------------------
    /**
     * Verifica la invariante de la clase. <b>inv: </b> <br>
     * El caso debe ser diferente de null<br>
     * El árbol es completo<br>
     * El tiempo de jugo es mayor a cero<br>
     * El árbol no debe tener repetidos<br>
     * La lista de sospechosos no puede ser null<br>
     * Debe haber un solo sospechoso que sea el ladrón<br>
     */
    private void verificarInvariante() {
        assert tiempoJuego > 0 : "El tiempo de juego debería ser entero mayor o "
                + "igual a cero";
        ...
        assert ((ciudadRaiz != null) ? ciudadRaiz.esCompleto() : true) : "El árbol "
                + "no es completo";
        int contador = 0;
        for(int i = 0; i < sospechosos.size(); i++) {
            if(((Sospechoso)sospechosos.get(i)).esLadron())
                contador++;
        }
        assert contador == 1 : "Debe existir un solo sospechoso. Usted definió " + 
                contador;
    }

    /**
     * Verifica que en el árbol no haya dos ciudades con el mismo nombre<br>
     * @return True en caso que haya ciudades repetidas en el árbol, false de lo 
     *         contrario.
     */
    @SuppressWarnings("rawtypes")
    ...
    /**
     * Cuenta el número de ciudades en el árbol que tienen el nombre dado como 
     * parámetro <br>
     * <b>pre: </b> El árbol de ciudades no es vacío
     * @param nombre nombre de la ciudad de la cual se está contando el número 
     *        de ocurrencias
     */
