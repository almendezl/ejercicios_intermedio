...
public class CampoMinado {
     ...
     * No se han destapado todas las casillas que no tienen bomba y no se ha
     * destapado una casilla con bomba
     */
    public static final int CONTINUA_JUEGO = 0;

    /**
     * Indica que el juego fue ganado porque se destaparon todas las casillas
     * que no están minadas.
     */
    public static final int JUEGO_GANADO = 1;

    /**
     * Indica que el juego fue perdido porque se destapó una casilla que estaba
     * minada
     */
    ...
    // -------------------------------------------------------------------------
    /**
     * Construye un nuevo juego de buscaminas con el tamaño del campo minado y
     * el número de minas especificados en el archivo de entrada.<br>
     * La distribución de las minas se realiza de manera aleatoria.<br>
     * <b>post: </b> Se construyó un nuevo campo minado del tamaño y número de
     * minas especificados en el archivo.<br>
     ...
     *
     * @param arch Es el archivo que contiene la información (filas, columnas y
     * número de minas) del campo minado. arch != null.
    ...
    /**
     * Retorna el número de minas que quedan por descubrir según el número de
     * marcas que se han puesto.
     *
    ...
    /**
     * Retorna una representación matricial de las casillas del campo minado
     * <br>
     * Si el juego no ha terminado se devuelven las casillas con los siguientes
     * estados: <br>
     * Las casillas que están tapadas se muestran tapadas a menos que estén
     * marcadas. <br>
     * De las casillas que están destapadas se muestra el número de minas
     * cercanas. Si el juego ya terminó se devuelven las casillas con los
     * siguientes estados: <br>
     * Si el jugador ganó, entonces se muestran las casillas igual que como se
     * mostrarían si el juego no hubiera terminado. <br>
     * Si el juego terminó y el jugador perdió entonces las casillas tapadas sin
     * marcar se muestran tapadas a menos que tengan una bomba, en cuyo caso
     * esta se debe mostrar; las casillas marcadas que tenían una bomba se
     * muestran marcadas, mientras que las marcadas que no tenían bomba se
     * muestran como marcadas equivocadas; para las casillasdestapadas se
     * muestra el número de bombas cercanas excepto para la casilla que contenía
     * la bomba, en la cual se muestra la bomba estallada.
     *
     ...
     * Las posiciones que están tapadas se muestran tapadas, a menos que estén
     * marcadas. <br>
     * De las posiciones que están destapadas se muestra el número de minas
     * cercanas.
     *
     * @return Matriz de objetos de tipo Casilla
     */
     ...
     * Si el juego terminó y el jugador ganó, entonces se muestran las casillas
     * igual que como se mostrarían si el juego no hubiera terminado. <br>
     * Si el juego terminó y el jugador perdió entonces las casillas tapadas sin
     * marcar se muestran tapadas a menos que tengan una bomba, en cuyo caso
     * esta se debe mostrar; las casillas marcadas marcadas que tenían una bomba
     * se muestran marcadas, mientras que las marcadas que no tenían bomba se
     * muestran como marcadas equivocadas; para las casillas destapadas se
     * muestra el número de bombas cercanas menos para la casilla que contenía
     * la bomba, en la cual se muestra la bomba estallada.
     *
     * @return Matriz de objetos de tipo Casilla
     */
    ...
    /**
     * Este método sirve para obtener el tipo correcto de una casilla según el
     * número de minas que haya cerca de ella.
     *
     * @param nMinas Es el número de minas que hay cerca de la casilla.
     * @param laFila Número de fila de la casilla.
     * @param laColumna Número de columna de la casilla.
     * @return Retorna una casilla del tipo correspondiente al número de minas
     * cercanas
     */
     ...
     * Si la casilla contenía una bomba entonces la bomba estalla y termina el
     * juego. <br>
     * Si la casilla no tenía una bomba, pero estaba cerca de una, entonces la
     * casilla correspondiente ahora deberá indicar el número de bombas
     * cercanas. <br>
     * <b>post: </b> La casilla está destapada y se actualizó el estado del
     * juego: número de minas restantes y el tiempo de juego.
     *
     ...
     * El resultado puede ser CONTINUA_JUEGO, si con la jugada el juego no
     * termina. <br>
     * Puede ser también JUEGO_GANADO o JUEGO_PERDIDO si con la jugada el juego
     * termina.
    ...
    /**
     * Calcula el tiempo actual cada vez que se hace una jugada. Para ello
     * utiliza un objeto de la clase Date.
     *
     */
    ...
    /**
     * Este método hace que todas las casillas que están cercanas a una casilla
     * sin minas cercanas queden destapadas. <br>
     * Una casilla está cercana a otra si entre ellas es posible establecer un
     * camino que pasa únicamente por casillas sin minas cercanas.
     *
     ...
     * El juego termina cuando todas las casillas sin minas están destapadas o
     * cuando hay una casilla minada que está destapada.
     *
     ...
     * Puede ser también JUEGO_GANADO o JUEGO_PERDIDO si el juego ya terminó y
     * el jugador ganó o perdió, respectivamente.
     */
    private int verificarFinal() {
        boolean sigueJuego = false;
        for (int i = 0; i < filas && !sigueJuego; i++) {
            for (int j = 0; j < columnas && !sigueJuego; j++) {
                if (casillasCampoMinado[i][j].darEstado() == Casilla.VACIA
                        && (casillasJugadas[i][j].darEstado() == Casilla.TAPADA
                        || casillasJugadas[i][j].darEstado() == Casilla.MARCADA)) {
                    sigueJuego = true;
                }
            }
        }
        ...
        if (!sigueJuego) {
            ...
            for (int i = 0; i < filas && !finJuego; i++) {
                for (int j = 0; j < columnas && !finJuego; j++) {
                    if (casillasCampoMinado[i][j].darEstado() == Casilla.MINADA
                            && casillasJugadas[i][j].darEstado() == Casilla.DESTAPADA) {
     ...
     * <b>pre: </b> La casilla no está marcada ni está destapada y el juego no
     * ha terminado <br>
     ...
     * @throws Exception Si la casilla ya está marcada o está destapada o el
     * juego ya terminó.
     */
    public void marcar(int x, int y) throws Exception {
        if (casillasJugadas[x][y].darEstado() == Casilla.MARCADA
                || casillasJugadas[x][y].darEstado() == Casilla.DESTAPADA) {
    ...
    public void desmarcar(int x, int y) throws Exception {
        if (casillasJugadas[x][y].darEstado() == Casilla.TAPADA
                || casillasJugadas[x][y].darEstado() == Casilla.DESTAPADA) {
    ...
    /**
     * Crea un nuevo campo minado de juego, tapando todas las casillas y
     * redistribuyendo las minas. El tiempo de inicio del juego se inicializa en
     * 0 <br>
     * <b>post: </b>El campo minado quedó inicializado, sin marcas, con todas
     * las casillas tapadas, con las minas distribuidas de forma aleatoria y con
     * el tiempo de inicio en 0. El número de minas destapadas también se
     * actualizó en cero.
     */
    @SuppressWarnings({"rawtypes", "unchecked"})
    ...
    /**
     * Retorna el tiempo total en segundos que fué necesario para resolver el
     * juego
     *
     * @return tiempo
     */
